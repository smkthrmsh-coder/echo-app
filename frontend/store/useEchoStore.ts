...
interface EchoStore {
  ...
  loginWithToken: (token: string, displayName?: string, userId?: string) => void;
  ...
  persist(
    (set, get) => ({
      ...
      signup: async (email, password, displayName) => {
        try {
          const resp = await apiSignup(email, password, displayName);
          setAuthToken(resp.access_token);
          set({
            isLoggedIn: true,
            screen: "home",
      ...
      loginWithToken: (token, displayName, userId) => {
        setAuthToken(token);
        set({
          isLoggedIn: true,
          screen: "home",
...
// Rehydrate auth token into the auth module on startup
if (typeof window !== "undefined") {
  const stored = useEchoStore.getState().authToken;
  if (stored) {
    setAuthToken(stored);
  }

  // Splash → login/home after 2s
  setTimeout(() => {
    const state = useEchoStore.getState();
    if (state.screen === "splash") {
      if (state.authToken && state.isLoggedIn) {
        useEchoStore.setState({ screen: "home" });
      } else {
        useEchoStore.setState({ screen: "login", isLoggedIn: false, authToken: null });
        setAuthToken(null);
      }
    }
  }, 2000);
